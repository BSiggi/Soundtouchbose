...
class FakeRequestError(Exception):
    def __init__(self, message: str, *, kind: str, operation: str = "select", status_code: int | None = None) -> None:
...
def test_error_details_extracts_operation_and_endpoint() -> None:
    details = error_details(FakeRequestError("SoundTouch HTTP 500", kind="http_status", operation="preset_write", status_code=500))
