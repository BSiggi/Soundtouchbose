...
function setupReadingProgress(){
  ...
  function getHeaderBottom(){
    var header = document.querySelector('header.header-bar')
      || document.querySelector('.masthead:not(#phantom)')
      || document.querySelector('.masthead')
      || document.querySelector('header');
    if (!header) return 0;
    var rect = header.getBoundingClientRect();
    var bottom = rect.bottom > 0 ? rect.bottom : 0;
    if (!logged) {
      console.log('fg-reading-progress header gefunden:', header.className, 'bottom:', bottom);
      logged = true;
    }
    return bottom;
  }
